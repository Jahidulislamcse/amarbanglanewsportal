...
class PendingController extends Controller
{
   public function __construct()
    ...
    public function index(){
        return view('user.pending.index');
    }
    
    public function categoryFilter($id){
        $datas = Language::find($id)->categories()->where('parent_id',NULL)->get();
        $output = '<option data-href="'.route('user.pending.datatables').'?category=&lang='.$id.'">All</option>';
        foreach($datas as $data){
            $output .= '<option data-href="'.route('user.pending.datatables').'?category='.$data->id.'">'.$data->title.'</option>';
        }
        return $output;
    }
}
