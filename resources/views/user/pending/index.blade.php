        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <ul class="links">
                    ...
                    <li>
                        <a href="{{ route('user.pending.index') }}">{{ __('Pending News') }}</a>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <div class="col-sm-2 m-3">
                ...
                <select  id="filter_lang">
                    @foreach ($languages as $language)
                        <option data-href="{{ route('user.pending.datatables') }}?lang={{ $language->id }}" value="{{ $language->id }}" {{ $language->is_default==1 ? 'selected':''}}>{{ $language->language }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-sm-2 m-3">
...
<script type="text/javascript">
    ...
    var table;
     ...
     table = $('#geniustable').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ route('user.pending.datatables') }}',
        columns: [
