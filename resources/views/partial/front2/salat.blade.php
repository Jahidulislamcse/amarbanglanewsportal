  ...
  <div class="pray-card" id="prayCard">
    <div class="header">
      <div>
        <div class="title">আজকের নামাজের সময়সূচি</div>
    ...
    <div class="grid" id="prayGrid" aria-live="polite">
      <!-- Row1: জোহর | বেলা | time -->
      <div class="cell"><div class="label">জোহর</div></div>
      <div class="cell"><div class="label">বেলা</div></div>
      <div class="cell"><div class="time" id="DhuhrTime">--:--</div></div>

      <!-- Row2: আসর | বিকেল | time -->
      <div class="cell"><div class="label">আসর</div></div>
      <div class="cell"><div class="label">বিকেল</div></div>
      <div class="cell"><div class="time" id="AsrTime">--:--</div></div>

      <!-- Row3: মাগরিব | সন্ন্যা | time -->
      <div class="cell"><div class="label">মাগরিব</div></div>
      <div class="cell"><div class="label">সন্ধ্যা</div></div>
      <div class="cell"><div class="time" id="MaghribTime">--:--</div></div>

      <!-- Row4: ইশা | রাত | time -->
      <div class="cell"><div class="label">ইশা</div></div>
      <div class="cell"><div class="label">রাত</div></div>
      <div class="cell"><div class="time" id="IshaTime">--:--</div></div>

      <!-- Row5: ফজর (ভোর) | ভোর | time -->
      <div class="cell"><div class="label">ফজর</div></div>
      <div class="cell"><div class="label">ভোর</div></div>
      <div class="cell"><div class="time" id="FajrTime">--:--</div></div>
    </div>

    <div class="foot">
      <div id="sunsetText">আজ সূর্যাস্ত - --:--</div>
      <div id="tomorrowSunriseText">আগামীকাল সূর্যোদয় - --:--</div>
    </div>
...
function updateCountdownUI(nextPrayer) {
  ...
  countdownEl.textContent = `🕰️ পরবর্তী নামাজ: ${enToBnPrayer(nextPrayer.name)} — ${secondsToHMS(seconds)}`;
}
...
function setSunTexts(meta) {
  ...
  if (sunset) sunsetEl.textContent = `আজ সূর্যাস্ত - ${formatTimeTo24(sunset)}`;
  else sunsetEl.textContent = `আজ সূর্যাস্ত - --:--`;
  tomorrowSunEl.textContent = `আগামীকাল সূর্যোদয় - --:--`;
...
async function loadWidget() {
  ...
  try {
    ...
    const dateStr = today.toLocaleDateString('bn-BD', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
    ...
    if (sunsetVal) {
      document.getElementById('sunsetText').textContent = `আজ সূর্যাস্ত - ${formatTimeTo24(sunsetVal)}`;
    }
    ...
    try {
      ...
      if (j2.status === 'success' && j2.timings && j2.timings.Fajr) {
        document.getElementById('tomorrowSunriseText').textContent = `আগামীকাল সূর্যোদয় - ${formatTimeTo24(j2.timings.Sunrise)}`;
      }
    } catch(e) {
      // ignore
    }
