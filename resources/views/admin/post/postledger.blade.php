            ...
            <div class="col-sm-2 m-3">
                ...
                <select  id="filter_lang">
                    @foreach ($languages as $language)
                        <option data-href="{{ route('post.postledgerdatatables') }}?lang={{ $language->id }}" value="{{ $language->id }}" {{ $language->is_default==1 ? 'selected':''}}>{{ $language->language }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-sm-2 m-3">
                            ...
                            <thead>
                                <tr>
                                   <th>{{ __('Image') }}</th>
									...
									<th>{{ __('Total Unique Views') }}</th>
									<th>{{ __('Total Unique Income') }}</th>
...
{{-- DELETE MODAL ENDS postledgerdatatables --}}
...
var table;
     ...
     table = $('#geniustable').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ route('post.postledgerdatatables') }}',
        columns: [
			...
			{data: 'total_unique_view',name: 'total_unique_view'},
...
<script src="{{asset('assets/admin/js/postledger.js')}}"></script>
