                    ...
                    <div class="gocover" style="background: url({{asset('assets/images/'.$gs->admin_loader)}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
                     <form id="geniusformdataedit"  action="{{ route('image.album.store')}}" method="POST"
                                ...
                                <div class="col-lg-12">
                                    <select name="language_id" id="almum_language_id">
                                        ...
                                        @foreach ($datas as $data)
                                            <option value="{{$data->id}}" {{ $default_language->id == $data->id ? 'selected' : ''}}>{{$data->language}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12">
                                   <input type="text" name="album_name" id="title" placeholder="{{ __('Album Name') }}" class="input-field">
                                </div>
                            </div>
							 <div class="row">
									<div class="col-lg-12">
							...
							<div class="row">
                                ...
                                <div class="col-lg-12">
                                   <input type="text" name="album_description" placeholder="{{ __('Album Description') }}" class="input-field">
                                </div>
                            </div>
...
<script>
  ...
  $(document).on('change',"#almum_language_id",function(){
        ...
        var x = $(this).val();
        var url = mainurl+'admin/add-album/language/'+x;
        $.ajax({
            type        : 'GET',
            url         : url,
            contentType : false,
