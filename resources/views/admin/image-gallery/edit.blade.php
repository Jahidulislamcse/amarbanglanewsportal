                    ...
                    <div class="gocover" style="background: url({{asset('assets/images/'.$gs->admin_loader)}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
                     <form id="geniusformdataedit"  action="{{ route('image.gallery.update',$data->id)}}" method="POST" enctype="multipart/form-data">
						 ...
						 <div class="row">
                            <div class="col-lg-12">
                                <div class="left-area">
                                    <h4 class="heading">{{ __('Staff Reporter') }} *</h4>
                                </div>
                            </div>
                            <div class="col-lg-12">
                                <select name="staff_id" id="staff_id">
                                    <option value="">{{__('Please Select Your Staf')}}</option>
									@foreach ($users as $key=>$user)
									  <option value="{{$key}}" {{ $key == $data->staff_id ? 'selected' : ''}}>{{$user}}</option>
									@endforeach
                                </select>
                            </div>
                        </div>
						 ...
						 <div class="row">
                                ...
                                <div class="col-lg-12">
                                   <input type="text" name="title" placeholder="{{ __('Title') }}" value="{{$data->title}}" class="input-field">
                                </div>
						</div>
