...
@section('content')
<input type="hidden" id="headerdata" value="{{ __('WARD / UNION') }}">
<div class="content-area">
    <div class="mr-breadcrumb">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="heading">{{ __('Wards / Unions') }}</h4>
                <ul class="links">
                    <li>
                    ...
                    <li>
                        <a href="{{ route('admin.wards.index') }}">{{ __('Wards / Unions') }}</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">{{ __('Add Ward / Union') }}</h5>
                ...
                <div class="add-product-content p-0 shadow-none">
                    <div class="product-description">
                        <div class="body-area shadow-none p-0">
                            ...
                            <form id="locationForm" action="{{ route('admin.wards.store') }}" method="POST">
                                {{ csrf_field() }}
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="left-area">
                                            <h4 class="heading">{{ __('Thana / Upazila') }} *</h4>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <select name="upazilla_id" id="upazilla_id_select" required class="form-control select">
                                            ...
                                            @foreach ($thanas as $thana)
                                                @php
                                                    ...
                                                    $districtName = $district ? $district->name . ($district->is_city_corporation ? ' (' . __('City Corp') . ')' : ' (' . __('District') . ')') : '';
                                                @endphp
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-lg-12">
                                        <input type="text" class="input-field" name="name" id="name_input" placeholder="{{ __('e.g. Ward 1') }}" required autocomplete="off">
                                    </div>
                                </div>
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-lg-12">
                                        <input type="text" class="input-field" name="bn_name" id="bn_name_input" placeholder="{{ __('e.g. ওয়ার্ড ১') }}" required autocomplete="off">
                                    </div>
                                </div>
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-lg-12">
                                        <input type="text" class="input-field" name="url" id="url_input" placeholder="{{ __('e.g. www.ward1.gov.bd') }}" autocomplete="off">
                                    </div>
                                </div>
...
<div class="modal fade-scale" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">
                    {{ __('You are about to delete this Ward/Union.') }}
                </p>
...
<script type="text/javascript">
    ...
    $(function () {
        $(".btn-area").append('<div class="col-sm-4 table-contents">' +
            ...
            '<i class="fas fa-plus"></i>{{ __('Add Ward / Union') }}' +
            '</a>' +
            '</div>');
    });
    ...
    $(document).on('click', '#btn-add-location', function() {
        $('#modalTitle').html('{{ __('Add Ward / Union') }}');
        $('#locationForm').attr('action', '{{ route('admin.wards.store') }}');
        $('#locationForm')[0].reset();
        $('#editId').val('');
        $('#upazilla_id_select').val('').trigger('change');
    ...
    $(document).on('click', '.edit-location-btn', function() {
        $('#modalTitle').html('{{ __('Edit Ward / Union') }}');
        
        var id = $(this).data('id');
        var upazilla_id = $(this).data('upazilla_id');
        var name = $(this).data('name');
        var bn_name = $(this).data('bn_name');
        var url = $(this).data('url');

        $('#locationForm').attr('action', mainurl + 'admin/wards/update/' + id);
        $('#editId').val(id);
        $('#upazilla_id_select').val(upazilla_id).trigger('change');
        $('#name_input').val(name);
