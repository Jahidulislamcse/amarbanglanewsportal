...
<a href="{{ route('frontend.index')}}"><i class="fa fa-home" aria-hidden="true"></i> 
							</a> <i class="fa fa-chevron-right"></i> <?php echo $title;?>
	</div>
	
	
	<div class="row" style="padding-bottom:15px;padding-top:10px;">
	<form class="archivenews" action="{{ route('front.news_archive') }}" method="get" enctype="multipart/form-data">
		<!-- Full Name -->
		<div class="col-md-9 col-sm-9">
			<input type="date" class="form-control" name="date" value="{{$date}}" placeholder="{{ __('Enter Date') }}" required>
		</div>
...
@if($posts && $posts->count() > 0)
    ...
    @foreach($posts as $key => $post)
        ...
        @if($key >= 0 && $key <= 5)
            ...
            @if($key == 5 || ($key == $posts->count() - 1 && $key < 6))
                </div>
            @endif
        @endif
            ...
            <div class="archive_page archive_back">
                <div class="row">
                    ...
                    <div class="col-md-8 col-sm-8">
                        ...
                        {{ strlen(strip_tags($post->description)) > 400 
                            ? convertUtf8(substr(strip_tags($post->description), 0, 400)) . '...' 
										...
										@endphp
											@if ($post)
												 @if ($post->image_big || $post->rss_image)
													@if ($post->image_big)
														<div class="small-img tab-border">
															<img width="600" height="337" data-src="{{asset('assets/images/post/'.$post->image_big)}}" class="lazy attachment-post-thumbnail size-post-thumbnail wp-post-image" alt="" decoding="async" ><h4 class="hadding_03"><a href="{{route('frontend.postBySubcategory.details',[$post->category->slug,$post->slug])}}">{{strlen($post->title)>40 ? mb_substr($post->title,0,40,"utf-8") : $post->title}} </a></h4>
														</div>
													@endif

													@if ($post->rss_image)
														<div class="small-img tab-border">
															<img width="600" height="337" data-src="{{asset('assets/images/post/'.$post->rss_image)}}" class="lazy attachment-post-thumbnail size-post-thumbnail wp-post-image" alt="" decoding="async" ><h4 class="hadding_03"><a href="{{route('frontend.postBySubcategory.details',[$post->category->slug,$post->slug])}}">{{strlen($post->title)>40 ? mb_substr($post->title,0,40,"utf-8") : $post->title}} </a></h4>
														</div>
													@endif
