    ...
    @foreach($posts as $key => $post)
        ...
        @if($key == 0)
            <div class="row archive_page">
                <div class="{{ $posts->count() == 1 ? 'col-md-12 col-sm-12' : 'col-md-8 col-sm-8' }}">
                    {{-- Show Image --}}
                    @if ($post->image_big)
                    ...
                    {!! strlen(convertUtf8(strip_tags($post->description))) > $description_first_size 
                        ? convertUtf8(substr(strip_tags($post->description), 0, $description_first_size)) . '...' 
                        : convertUtf8(strip_tags($post->description)) !!}
                    <span style="text-align:right; display:block;">
                        <a href="{{ route('frontend.postBySubcategory.details', [$post->category->slug, $post->slug]) }}">{{ __('Read More') }}</a>
                    </span>
                </div>

                {{-- If there is only 1 post, close the row here --}}
                @if($posts->count() == 1)
                    </div>
                @endif
        @endif
...
@endif
				...
				@endif
								...
								<div class="add"><?php echo $Category_Page_Bottom_Size_360x300 ?? null;?></div>
